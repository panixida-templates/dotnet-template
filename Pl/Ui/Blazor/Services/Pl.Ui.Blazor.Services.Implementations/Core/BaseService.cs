...
{
    public async Task<TViewModel> GetAsync(TId id, TConvertParams convertParams, CancellationToken cancellationToken = default)
    {
        var dto = await apiHttpClient.GetAsync<TDto>(
    ...
    public async Task<SearchResult<TViewModel>> GetAsync(TSearchParams searchParams, TConvertParams convertParams, CancellationToken cancellationToken = default)
    {
        var searchResult = await apiHttpClient.GetAsync<SearchResult<TDto>>(
    ...
    public Task<TId> CreateAsync(TViewModel viewModel, CancellationToken cancellationToken = default)
    {
        return apiHttpClient.PostAsync<TDto, TId>(
    ...
    {
        return apiHttpClient.PutAsync<TDto, NoContent>(
            endpoint: TEndpoint.ById(id),
            request: toDto(viewModel),
            cancellationToken: cancellationToken);
    }
